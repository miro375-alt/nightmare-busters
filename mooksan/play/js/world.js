...
/* ══ 복도 오브젝트 조사 (2026 고정) ══ */
const HTEXT={
 cabinet:['복도 사물함이다.','내 건 2층에 있다.','여긴 전부 모르는 이름이다.'],
 locker:['철제 캐비닛이다.','문에 이름표가 붙어 있다.','「2-4 김」…… 다음 글자가 지워졌다.',
   '지워진 게 아니라, 처음부터 거기까지만 쓴 것 같다.'],
 shelf:['학급문고다.','문제집과 참고서가 꽂혀 있다.','전부 9월까지 진도가 나가 있다.',
   '10월 단원은 아무도 안 폈다.'],
 shelf2:['책장이다.','학급문고 명부가 붙어 있다.','대출 칸이 전부 채워져 있는데',
   '반납 칸은 전부 비어 있다.'],
 rack:['교구 진열대다.','실습 재료가 종류별로 담겨 있다.','상자마다 수량이 적혀 있다.',
   '……하나도 안 줄었다.'],
 notice:['학급 게시판이다.','시간표, 급훈, 자리 배치도.','자리 배치도에 내 자리가 있다.',
   '이 반은 우리 반이 아닌데.'],
 cork:['알림판이다.','「야간자율학습 중 복도 통행 자제」','밑에 작게 덧붙여 있다.',
   '「특히 3층」'],
 map:['세계지도다.','오래된 지도다. 나라 이름이 몇 개 다르다.',
   '동해 표기 옆에 누가 볼펜으로 점을 찍어 놨다.','점은 바다 한가운데다.'],
 board_s:['작은 칠판이다.','오늘의 전달사항이 적혀 있다.','「하교 시 반드시 짝과 함께」',
   '……야자 하교는 원래 혼자 한다.'],
 hyd:['소화기다.','압력계 바늘이 초록 칸에 있다.','점검 스티커가 붙어 있다.',
   '마지막 점검란이 이번 달이다.','서명은 있는데 읽을 수가 없다.'],
 cooler:['정수기다.','전원은 들어와 있다.','냉수도 나온다.','종이컵이 하나도 없다.','컵꽂이만 남아 있다.'],
 bin:['쓰레기통이다.','비어 있다.','비운 게 아니라 한 번도 안 쓴 것 같다.'],
 plant:['화분이다.','만져보니 조화(造花)다.','그런데 먼지가 없다.'],
 globe:['지구본이다.','복도에 지구본이 왜 있지.','돌려 봤다.','한 바퀴 돌고, 멈추는 자리가 같다.'],
 rack2:['교구 진열대다.','실험 기구가 번호순으로 놓여 있다.','번호가 하나 비어 있다.','12번.'],
 shelf3:['유리문 책장이다.','안쪽 유리에 김이 서려 있다.','복도는 이렇게 추운데.'],
 shelf4:['책장이다.','아래 칸에 분실물이 모여 있다.','실내화, 우산, 도시락통.',
   '전부 이름표가 붙어 있다.','같은 이름이다.'],
 ladder:['접이식 사다리다.','천장 점검구 아래 세워져 있다.','점검구는 닫혀 있다.',
   '……사다리 발판에 먼지가 없다.'],
 tallboard:['이동식 칠판이다.','복도에 나와 있다.','판서가 반쯤 지워져 있다.',
   '남은 글자는 「3층은」까지다.'],
};
export function hallProp(hp){
  const key=hp.replace(/#\d+$/,''), k='h/'+key;
  ev('prop',{key:k});
  if(!S.found[k]){ S.found[k]=1; }
  say(HTEXT[key] || ['……별것 없다.']);        // 미등록 소품이라도 크래시하지 않는다 (D99)
}
